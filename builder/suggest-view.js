...
//               the moves, the threats it helps against, the estimated score changes,
//               the score and Use
//   opened      Score changes (the estimate the ranking uses and, once checked, the
//               full Team Evaluation of the team it would make), Why it fits, Team
//               Building Check changes, Critical threats with their calcs, the set,
...
export function suggestionRow(row, helpers, result = null) {
  ...
  details.append(h("summary", {},
    ...
    h("div", { class: "bd-sg-main" },
      ...
      helps.length ? h("p", { class: "bd-sg-helps" }, `Helps vs ${helps.join(", ")}`) : null,
...
function breakdown(row, helpers, result) {
  ...
  // 4. Critical threats: what it answers (with the calcs), how the threat scores move.
  body.append(threatSection(row, projected, waiting, helpers));
  ...
  if (ledger.length) {
    ...
    body.append(section("How the score is built",
      h("ul", { class: "bd-sg-ledger" }, ledger.map((item) => h("li", { class: item.value !== undefined ? "base" : tone(item.delta) },
      ...
      h("p", { class: "bd-note" }, [
        ...
        row.speed_conflict ? "This one has such a conflict, so it is listed later." : row.role_fixes ? "This one fixes Speed Control, so it is listed earlier." : "",
      ].filter(Boolean).join(" "))));
  }
  return body;
}
...
function threatSection(row, projected, waiting, helpers = {}) {
  const { one } = namers(helpers);
  const parts = [];
  if (projected) {
    if (projected.answers?.length) {
      parts.push(h("p", { class: "bd-sg-sub" }, `Answers in the full Team Evaluation (${projected.answers.length})`));
      parts.push(h("div", { class: "bd-sg-answers" }, projected.answers.map((a) => h("div", { class: "bd-sg-answer" },
        h("div", { class: "bd-sg-answer-head" }, h("strong", {}, one(a.threat)), h("span", { class: "bd-note" }, `threat score ${fixed1(a.score)}`)),
        ...calcPair(a.incoming, a.outgoing, a.first, one)))));
    } else {
      parts.push(h("p", { class: "bd-note" }, "It wins none of the critical matchups on its own in the full Team Evaluation; its value is in the checks and scores above."));
    }
    const moves = [
      ...(projected.lowered || []).map((t) => h("li", { class: "up" }, h("span", {}, one(t.threat)), h("b", {}, t.after === null ? `${fixed1(t.before)} → no longer critical` : `${fixed1(t.before)} → ${fixed1(t.after)}`))),
      ...(projected.raised || []).map((t) => h("li", { class: "down" }, h("span", {}, one(t.threat)), h("b", {}, t.before === null ? `new, ${fixed1(t.after)}` : `${fixed1(t.before)} → ${fixed1(t.after)}`))),
    ];
    if (moves.length) {
      parts.push(h("p", { class: "bd-sg-sub" }, `Threat scores after adding it · critical threats ${projected.critical.before} → ${projected.critical.after}`));
      parts.push(h("ul", { class: "bd-sg-threat-moves" }, moves));
      parts.push(h("p", { class: "bd-note" }, "Lower is better: a threat score says how hard that Pokémon is on the team (70 and above is a serious threat)."));
    }
  } else {
    const backed = (row.answer_calcs || []).filter((c) => c.kept && (c.incoming || c.outgoing));
    if (backed.length) {
      parts.push(h("p", { class: "bd-sg-sub" }, "Answers on its most common set"));
      parts.push(h("div", { class: "bd-sg-answers" }, backed.map((c) => h("div", { class: "bd-sg-answer" }, h("div", { class: "bd-sg-answer-head" }, h("strong", {}, one(c.threat))), ...calcPair(c.incoming, c.outgoing, "outgoing", one)))));
    }
    parts.push(h("p", { class: "bd-note" }, waiting ? "Checking against the full Team Evaluation…" : "Not checked against the full Team Evaluation."));
  }
  const struck = (row.answer_calcs || []).filter((c) => !c.kept && (c.incoming || c.outgoing));
  if (struck.length) {
    parts.push(h("p", { class: "bd-sg-sub" }, "Type matchup only"));
    parts.push(h("p", { class: "bd-note" }, "Its typing looks good into these, but the calc says it loses the matchup, so they do not count as answers:"));
    parts.push(h("div", { class: "bd-sg-answers" }, struck.map((c) => h("div", { class: "bd-sg-answer struck" }, h("div", { class: "bd-sg-answer-head" }, h("strong", {}, one(c.threat))), ...calcPair(c.incoming, c.outgoing, "outgoing", one)))));
  }
  return section("Critical threats", ...parts);
}
